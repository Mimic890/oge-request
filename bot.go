package main

import (
	"fmt"
	"log"
	"net/http"
	"net/url"
	"os"
	"regexp"
	"runtime"
	"strings"
	"sync"
	"time"

	tgbotapi "github.com/go-telegram-bot-api/telegram-bot-api/v5"
)

var codePattern = regexp.MustCompile(`^\d{4}-\d{4}-\d{4}$`)

type Bot struct {
	api               *tgbotapi.BotAPI
	cfg               Config
	store             *Storage
	adminUsername      string
	mu                sync.Mutex
	waiting           map[int64]bool
	adminWaiting      map[int64]bool
	adminBroadcastLvl map[int64]string
	stopChan          chan struct{}
}

func NewBot(cfg Config, store *Storage) (*Bot, error) {
	api, err := tgbotapi.NewBotAPI(cfg.TelegramToken)
	if err != nil {
		return nil, fmt.Errorf("telegram: %w", err)
	}

	proxyURL := os.Getenv("HTTPS_PROXY")
	if proxyURL == "" {
		proxyURL = os.Getenv("HTTP_PROXY")
	}
	if proxyURL != "" {
		parsed, err := url.Parse(proxyURL)
		if err == nil {
			api.Client = &http.Client{
				Transport: &http.Transport{
					Proxy: http.ProxyURL(parsed),
				},
			}
			log.Printf("telegram proxy: %s", proxyURL)
		}
	}

	log.Printf("authorized as @%s", api.Self.UserName)
	b := &Bot{
		api:               api,
		cfg:               cfg,
		store:             store,
		waiting:           make(map[int64]bool),
		adminWaiting:      make(map[int64]bool),
		adminBroadcastLvl: make(map[int64]string),
		stopChan:          make(chan struct{}),
	}

	b.setCommands()
	b.resolveAdminUsername()
	go b.poll()
	return b, nil
}

func (b *Bot) resolveAdminUsername() {
	if b.cfg.AdminID == 0 {
		return
	}
	chat, err := b.api.GetChat(tgbotapi.ChatInfoConfig{
		ChatConfig: tgbotapi.ChatConfig{ChatID: b.cfg.AdminID},
	})
	if err != nil {
		log.Printf("cannot resolve admin username: %v", err)
		return
	}
	if chat.UserName != "" {
		b.adminUsername = "@" + chat.UserName
		log.Printf("admin: %s (id: %d)", b.adminUsername, b.cfg.AdminID)
	} else {
		log.Printf("admin id: %d (no username)", b.cfg.AdminID)
	}
}

func (b *Bot) adminContact() string {
	if b.adminUsername != "" {
		return b.adminUsername
	}
	return fmt.Sprintf("id:%d", b.cfg.AdminID)
}

func (b *Bot) setCommands() {
	cmds := []tgbotapi.BotCommand{
		{Command: "start", Description: "Запуск и главное меню"},
		{Command: "repo", Description: "Ссылка на код бота"},
	}
	if b.cfg.AdminID != 0 {
		cmds = append(cmds, tgbotapi.BotCommand{
			Command: "status", Description: "Статус бота (админ)"},
		)
	}
	cfg := tgbotapi.NewSetMyCommands(cmds...)
	b.api.Request(cfg)
}

func (b *Bot) poll() {
	u := tgbotapi.NewUpdate(0)
	u.Timeout = 30
	updates := b.api.GetUpdatesChan(u)
	for {
		select {
		case upd, ok := <-updates:
			if !ok {
				return
			}
			if upd.CallbackQuery != nil {
				b.onCallback(upd.CallbackQuery)
			} else if upd.Message != nil && upd.Message.IsCommand() {
				b.onCommand(upd.Message)
			} else if upd.Message != nil {
				b.onText(upd.Message)
			}
		case <-b.stopChan:
			b.api.StopReceivingUpdates()
			return
		}
	}
}

func (b *Bot) Stop() {
	close(b.stopChan)
}

func (b *Bot) isAdmin(uid int64) bool {
	return b.cfg.AdminID != 0 && uid == b.cfg.AdminID
}

func (b *Bot) touchUser(uid int64, username string) {
	b.store.UpdateLastActive(uid)
	b.store.UpdateUsername(uid, username)
}

func userLog(u *tgbotapi.User) string {
	if u.UserName != "" {
		return fmt.Sprintf("@%s (%d)", u.UserName, u.ID)
	}
	return fmt.Sprintf("uid:%d", u.ID)
}

func (b *Bot) onCommand(msg *tgbotapi.Message) {
	uid := msg.From.ID
	b.touchUser(uid, msg.From.UserName)
	log.Printf("[cmd] /%s from %s", msg.Command(), userLog(msg.From))

	b.mu.Lock()
	b.adminWaiting[uid] = false
	delete(b.adminBroadcastLvl, uid)
	b.waiting[uid] = false
	b.mu.Unlock()

	switch msg.Command() {
	case "start":
		b.cmdStart(msg)
	case "status":
		if b.isAdmin(uid) {
			b.cmdStatus(msg)
		} else {
			b.reply(msg.Chat.ID, "Нет доступа.")
		}
	case "repo":
		b.reply(msg.Chat.ID,
			"Исходный код бота открыт:\n"+
				"<a href=\""+repoURL+"\">"+repoMessage+"</a>")
	default:
		b.reply(msg.Chat.ID, "Неизвестная команда. Нажмите /start")
	}
}

func (b *Bot) cmdStart(msg *tgbotapi.Message) {
	uid := msg.From.ID
	chatID := msg.Chat.ID

	users := b.store.LoadUsers()
	_, hasCode := users[uid]
	kb := mainKeyboard(b.isAdmin(uid), hasCode)

	if u, ok := users[uid]; ok {
		masked := maskCode(u.Code)
		interval := u.GetInterval()
		txt := msgWelcomeRegistered(b.cfg.SiteDomain, masked, interval)
		b.sendWithInline(chatID, txt, kb)
	} else {
		b.sendWithInline(chatID, msgWelcome(b.cfg.SiteDomain), kb)
	}
}

func (b *Bot) onCallback(q *tgbotapi.CallbackQuery) {
	b.api.Request(tgbotapi.NewCallback(q.ID, ""))

	uid := q.From.ID
	chatID := q.Message.Chat.ID
	msgID := q.Message.MessageID
	data := q.Data
	b.touchUser(uid, q.From.UserName)
	log.Printf("[btn] %s from %s", data, userLog(q.From))

	switch data {
	case "set_code":
		users := b.store.LoadUsers()
		if _, already := users[uid]; !already && b.cfg.MaxUsers > 0 && len(users) >= b.cfg.MaxUsers {
			b.edit(chatID, msgID, "Лимит пользователей исчерпан. Попробуйте позже.", mainKeyboard(b.isAdmin(uid), false))
			return
		}
		b.mu.Lock()
		b.waiting[uid] = true
		b.mu.Unlock()
		b.edit(chatID, msgID, msgSetCode(), backToMenu())

	case "check":
		b.checkAndSend(chatID, msgID, uid)

	case "disable":
		kb := tgbotapi.NewInlineKeyboardMarkup(
			tgbotapi.NewInlineKeyboardRow(
				tgbotapi.NewInlineKeyboardButtonData("✅ Да, удалить", "disable_confirm"),
				tgbotapi.NewInlineKeyboardButtonData("❌ Нет, назад", "menu"),
			),
		)
		b.edit(chatID, msgID, msgDeleteConfirm(), &kb)

	case "disable_confirm":
		b.store.RemoveUser(uid)
		b.edit(chatID, msgID, msgCodeDeleted(), nil)

	case "menu":
		users := b.store.LoadUsers()
		_, hasCode := users[uid]
		kb := mainKeyboard(b.isAdmin(uid), hasCode)
		if u, ok := users[uid]; ok {
			masked := maskCode(u.Code)
			interval := u.GetInterval()
			b.edit(chatID, msgID, msgWelcomeRegistered(b.cfg.SiteDomain, masked, interval), kb)
		} else {
			b.edit(chatID, msgID, msgWelcome(b.cfg.SiteDomain), kb)
		}

	case "notif_settings":
		users := b.store.LoadUsers()
		if _, hasCode := users[uid]; !hasCode {
			b.edit(chatID, msgID, "Сначала настройте код участника.", mainKeyboard(b.isAdmin(uid), false))
			return
		}
		enabled := b.store.GetNotifEnabled(uid)
		interval := b.store.GetCheckInterval(uid)
		kb := notifSettingsKeyboard(enabled, interval)
		b.edit(chatID, msgID, msgNotifSettings(enabled, interval), kb)

	case "notif_toggle":
		enabled := b.store.GetNotifEnabled(uid)
		b.store.SetNotifEnabled(uid, !enabled)
		newEnabled := !enabled
		kb := notifSettingsKeyboard(newEnabled, b.store.GetCheckInterval(uid))
		b.edit(chatID, msgID, msgNotifToggled(newEnabled)+"\n\n"+msgNotifSettings(newEnabled, b.store.GetCheckInterval(uid)), kb)

	case "notif_5":
		b.store.SetCheckInterval(uid, 5)
		enabled := b.store.GetNotifEnabled(uid)
		kb := notifSettingsKeyboard(enabled, 5)
		b.edit(chatID, msgID, msgIntervalChanged(5)+"\n\n"+msgNotifSettings(enabled, 5), kb)

	case "notif_10":
		b.store.SetCheckInterval(uid, 10)
		enabled := b.store.GetNotifEnabled(uid)
		kb := notifSettingsKeyboard(enabled, 10)
		b.edit(chatID, msgID, msgIntervalChanged(10)+"\n\n"+msgNotifSettings(enabled, 10), kb)

	case "notif_15":
		b.store.SetCheckInterval(uid, 15)
		enabled := b.store.GetNotifEnabled(uid)
		kb := notifSettingsKeyboard(enabled, 15)
		b.edit(chatID, msgID, msgIntervalChanged(15)+"\n\n"+msgNotifSettings(enabled, 15), kb)

	case "admin_status":
		if b.isAdmin(uid) {
			b.edit(chatID, msgID, b.buildStatusText(), adminStatusMenu())
		} else {
			b.edit(chatID, msgID, "Нет доступа.", mainKeyboard(false, false))
		}

	case "admin_users":
		if b.isAdmin(uid) {
			b.edit(chatID, msgID, b.buildUsersText(), adminUsersMenu())
		} else {
			b.edit(chatID, msgID, "Нет доступа.", mainKeyboard(false, false))
		}

	case "admin_broadcast":
		if b.isAdmin(uid) {
			b.mu.Lock()
			b.adminWaiting[uid] = false
			delete(b.adminBroadcastLvl, uid)
			b.mu.Unlock()
			b.edit(chatID, msgID, msgBroadcastPrompt(), broadcastImportanceKeyboard())
		}

	case "broadcast_low", "broadcast_mid", "broadcast_high":
		if b.isAdmin(uid) {
			level := strings.TrimPrefix(data, "broadcast_")
			b.mu.Lock()
			b.adminWaiting[uid] = true
			b.adminBroadcastLvl[uid] = level
			b.mu.Unlock()
			kb := tgbotapi.NewInlineKeyboardMarkup(
				tgbotapi.NewInlineKeyboardRow(
					tgbotapi.NewInlineKeyboardButtonData("◀️ Назад", "admin_broadcast"),
				),
			)
			b.edit(chatID, msgID, msgBroadcastEnter(), &kb)
		}
	}
}

func (b *Bot) onText(msg *tgbotapi.Message) {
	uid := msg.From.ID
	chatID := msg.Chat.ID
	b.touchUser(uid, msg.From.UserName)
	log.Printf("[text] %q from %s", msg.Text, userLog(msg.From))

	b.mu.Lock()
	adminW := b.adminWaiting[uid]
	b.adminWaiting[uid] = false
	adminLvl := b.adminBroadcastLvl[uid]
	delete(b.adminBroadcastLvl, uid)
	waiting := b.waiting[uid]
	b.waiting[uid] = false
	b.mu.Unlock()

	if adminW && b.isAdmin(uid) {
		b.adminBroadcast(chatID, uid, msg.Text, adminLvl)
		return
	}

	if waiting {
		code := strings.TrimSpace(msg.Text)
		if !codePattern.MatchString(code) {
			b.reply(chatID, msgInvalidCode())
			b.mu.Lock()
			b.waiting[uid] = true
			b.mu.Unlock()
			return
		}
		log.Printf("[code] user %s set code %s", userLog(msg.From), maskCode(code))
		b.store.SaveUser(uid, code, msg.From.UserName)
		b.reply(chatID, msgCodeSaved())
		b.handleCheck(chatID, uid)
		return
	}
}

func (b *Bot) adminBroadcast(chatID int64, uid int64, text string, level string) {
	users := b.store.LoadUsers()
	total := len(users)

	header := broadcastHeader(level)
	fullText := header + "\n\n" + text

	sent := 0
	for targetUID := range users {
		if err := b.sendBroadcast(targetUID, fullText); err != nil {
			log.Printf("[broadcast] failed to send to uid=%d: %v", targetUID, err)
		} else {
			sent++
		}
	}
	log.Printf("[broadcast] admin %s level=%s sent to %d/%d users", userLog(&tgbotapi.User{ID: uid}), level, sent, total)
	b.sendWithInline(chatID, msgBroadcastSent(total, sent), backToMenu())
}

func broadcastHeader(level string) string {
	switch level {
	case "high":
		return "<b>🚨 Срочное оповещение от администратора</b>"
	case "mid":
		return "<b>⚠️ Важное оповещение от администратора</b>"
	default:
		return "<b>📢 Оповещение от администратора</b>"
	}
}

func (b *Bot) sendBroadcast(uid int64, text string) error {
	msg := tgbotapi.NewMessage(uid, text)
	msg.ParseMode = "HTML"
	msg.DisableWebPagePreview = true
	_, err := b.api.Send(msg)
	return err
}

func (b *Bot) handleCheck(chatID int64, uid int64) {
	users := b.store.LoadUsers()
	u, ok := users[uid]
	if !ok {
		b.sendWithInline(chatID, "Сначала настройте код участника.", mainKeyboard(false, false))
		return
	}

	existing := b.store.GetUserResultsOrdered(uid)
	if len(existing) > 0 {
		lastUpdated := b.store.GetLastUpdated(uid)
		text := FormatOrderedResults(existing, lastUpdated)
		b.sendWithInline(chatID, text, resultMenu())
		return
	}

	ordered, err := FetchResultsFresh(u.Code)
	if err != nil {
		b.sendWithInline(chatID, msgSiteUnavailable(b.adminContact()), mainKeyboard(b.isAdmin(uid), true))
		return
	}
	if ordered.Len() == 0 {
		b.sendWithInline(chatID, msgNoResults(), resultMenu())
		return
	}

	changed := b.store.UpdateUserResultsOrdered(uid, ordered)
	text := FormatOrderedResults(ordered.Subjects, time.Now())
	if len(changed) > 0 {
		text = msgUpdateHeader() + text
	}
	b.sendWithInline(chatID, text, resultMenu())
}

func (b *Bot) checkAndSend(chatID int64, msgID int, uid int64) {
	users := b.store.LoadUsers()
	u, ok := users[uid]
	if !ok {
		b.edit(chatID, msgID, "Сначала настройте код участника.", mainKeyboard(false, false))
		return
	}

	b.edit(chatID, msgID, msgChecking(), nil)

	ordered, err := FetchResultsFresh(u.Code)
	if err != nil {
		stats.RecordError()
		b.store.IncrementErrorCount(uid)
		log.Printf("[check] uid=%d code=%s error: %v", uid, maskCode(u.Code), err)
		notifyAdminError(b, uid, u.Code, u.Username, err)
		b.edit(chatID, msgID, msgSiteUnavailable(b.adminContact()), resultMenu())
		return
	}
	if ordered.Len() == 0 {
		b.edit(chatID, msgID, msgNoResults(), resultMenu())
		return
	}

	b.store.ResetErrorCount(uid)
	changed := b.store.UpdateUserResultsOrdered(uid, ordered)
	text := FormatOrderedResults(ordered.Subjects, time.Now())
	if len(changed) > 0 {
		text = msgUpdateHeader() + text
	}
	b.edit(chatID, msgID, text, resultMenu())
}

func (b *Bot) cmdStatus(msg *tgbotapi.Message) {
	b.sendWithInline(msg.Chat.ID, b.buildStatusText(), adminStatusMenu())
}

func (b *Bot) send(chatID int64, text string, kb *tgbotapi.InlineKeyboardMarkup) {
	msg := tgbotapi.NewMessage(chatID, text)
	msg.ParseMode = "HTML"
	msg.DisableWebPagePreview = true
	if kb != nil {
		msg.ReplyMarkup = kb
	}
	if _, err := b.api.Send(msg); err != nil {
		log.Printf("send error: %v", err)
	}
}

func (b *Bot) sendWithInline(chatID int64, text string, kb *tgbotapi.InlineKeyboardMarkup) {
	b.send(chatID, text, kb)
}

func (b *Bot) reply(chatID int64, text string) {
	b.send(chatID, text, nil)
}

func (b *Bot) sendSilent(chatID int64, text string) {
	msg := tgbotapi.NewMessage(chatID, text)
	msg.ParseMode = "HTML"
	msg.DisableWebPagePreview = true
	msg.DisableNotification = true
	if _, err := b.api.Send(msg); err != nil {
		log.Printf("send error: %v", err)
	}
}

func (b *Bot) edit(chatID int64, messageID int, text string, kb *tgbotapi.InlineKeyboardMarkup) {
	msg := tgbotapi.NewEditMessageText(chatID, messageID, text)
	msg.ParseMode = "HTML"
	msg.DisableWebPagePreview = true
	if kb != nil {
		msg.ReplyMarkup = kb
	}
	if _, err := b.api.Send(msg); err != nil {
		if !strings.Contains(err.Error(), "message is not modified") {
			log.Printf("edit error: %v", err)
		}
	}
}

func maskCode(code string) string {
	if len(code) < 8 {
		return "****"
	}
	return code[:4] + "****" + code[len(code)-4:]
}

func mainKeyboard(isAdmin bool, hasCode bool) *tgbotapi.InlineKeyboardMarkup {
	rows := [][]tgbotapi.InlineKeyboardButton{
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("📊 Результаты", "check"),
		),
	}
	if hasCode {
		rows = append(rows,
			tgbotapi.NewInlineKeyboardRow(
				tgbotapi.NewInlineKeyboardButtonData("⚙️ Настройки уведомлений", "notif_settings"),
			),
			tgbotapi.NewInlineKeyboardRow(
				tgbotapi.NewInlineKeyboardButtonData("🔑 Изменить код", "set_code"),
				tgbotapi.NewInlineKeyboardButtonData("🗑 Удалить код", "disable"),
			),
		)
	} else {
		rows = append(rows,
			tgbotapi.NewInlineKeyboardRow(
				tgbotapi.NewInlineKeyboardButtonData("🔑 Настроить код", "set_code"),
			),
		)
	}
	if isAdmin {
		rows = append(rows,
			tgbotapi.NewInlineKeyboardRow(
				tgbotapi.NewInlineKeyboardButtonData("📈 Статус", "admin_status"),
				tgbotapi.NewInlineKeyboardButtonData("👥 Юзеры", "admin_users"),
			),
			tgbotapi.NewInlineKeyboardRow(
				tgbotapi.NewInlineKeyboardButtonData("📢 Рассылка", "admin_broadcast"),
			),
		)
	}
	kb := tgbotapi.NewInlineKeyboardMarkup(rows...)
	return &kb
}

func notifSettingsKeyboard(enabled bool, interval int) *tgbotapi.InlineKeyboardMarkup {
	toggleText := "🔕 Выключить уведомления"
	if !enabled {
		toggleText = "🔔 Включить уведомления"
	}

	interval5 := "5 мин"
	interval10 := "10 мин"
	interval15 := "15 мин"
	if interval == 5 {
		interval5 = "5 мин ✓"
	}
	if interval == 10 {
		interval10 = "10 мин ✓"
	}
	if interval == 15 {
		interval15 = "15 мин ✓"
	}

	kb := tgbotapi.NewInlineKeyboardMarkup(
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData(toggleText, "notif_toggle"),
		),
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData(interval5, "notif_5"),
			tgbotapi.NewInlineKeyboardButtonData(interval10, "notif_10"),
			tgbotapi.NewInlineKeyboardButtonData(interval15, "notif_15"),
		),
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("◀️ Назад", "menu"),
		),
	)
	return &kb
}

func adminStatusMenu() *tgbotapi.InlineKeyboardMarkup {
	kb := tgbotapi.NewInlineKeyboardMarkup(
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("🔄 Обновить", "admin_status"),
		),
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("👥 Юзеры", "admin_users"),
		),
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("🏠 Меню", "menu"),
		),
	)
	return &kb
}

func adminUsersMenu() *tgbotapi.InlineKeyboardMarkup {
	kb := tgbotapi.NewInlineKeyboardMarkup(
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("🔄 Обновить", "admin_users"),
		),
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("📈 Статус", "admin_status"),
		),
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("🏠 Меню", "menu"),
		),
	)
	return &kb
}

func resultMenu() *tgbotapi.InlineKeyboardMarkup {
	kb := tgbotapi.NewInlineKeyboardMarkup(
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("🔄 Проверить снова", "check"),
		),
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("🏠 Меню", "menu"),
		),
	)
	return &kb
}

func backToMenu() *tgbotapi.InlineKeyboardMarkup {
	kb := tgbotapi.NewInlineKeyboardMarkup(
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("◀️ Назад", "menu"),
		),
	)
	return &kb
}

func broadcastImportanceKeyboard() *tgbotapi.InlineKeyboardMarkup {
	kb := tgbotapi.NewInlineKeyboardMarkup(
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("🟢 Обычное", "broadcast_low"),
			tgbotapi.NewInlineKeyboardButtonData("🟡 Важное", "broadcast_mid"),
			tgbotapi.NewInlineKeyboardButtonData("🔴 Срочное", "broadcast_high"),
		),
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("◀️ Назад", "menu"),
		),
	)
	return &kb
}

func (b *Bot) buildStatusText() string {
	totalUsers, activeUsers := b.store.ActiveUsers()
	uptime := stats.UptimeDuration()
	hours := int(uptime.Hours())
	mins := int(uptime.Minutes()) % 60
	alloc, sys := GetRAM()

	limitStr := "без лимита"
	if b.cfg.MaxUsers > 0 {
		limitStr = fmt.Sprintf("%d", b.cfg.MaxUsers)
	}

	return msgAdminStatus(
		b.cfg.SiteDomain,
		hours, mins,
		totalUsers, activeUsers,
		limitStr,
		stats.SiteVisitsToday(), stats.TotalSiteVisits(),
		FormatBytes(stats.TotalBytes()),
		FormatBytes(int64(alloc)), FormatBytes(int64(sys)),
		runtime.NumGoroutine(),
		stats.ErrorsToday(),
	)
}

func (b *Bot) buildUsersText() string {
	users := b.store.LoadUsersSortedWithIDs()
	if len(users) == 0 {
		return msgAdminUsersEmpty()
	}
	var sb strings.Builder
	sb.WriteString(msgAdminUsersHeader())
	for _, u := range users {
		masked := maskCode(u.Entry.Code)
		interval := u.Entry.GetInterval()
		notif := "вкл"
		if !u.Entry.Enabled {
			notif = "выкл"
		}
		name := fmt.Sprintf("%d", u.ID)
		if u.Entry.Username != "" {
			name = fmt.Sprintf("%d - @%s", u.ID, u.Entry.Username)
		}
		sb.WriteString(fmt.Sprintf("<code>%s</code>\n", name))
		codeLine := fmt.Sprintf("  Код: %s", masked)
		if u.Entry.ErrorCount > 0 {
			codeLine += fmt.Sprintf(" 🔴%d", u.Entry.ErrorCount)
		}
		sb.WriteString(codeLine + "\n")
		sb.WriteString(fmt.Sprintf("  Интервал: %d мин | Уведомления: %s\n\n", interval, notif))
	}
	return sb.String()
}

func FormatBytes(b int64) string {
	const (
		KB = 1024
		MB = 1024 * KB
		GB = 1024 * MB
	)
	switch {
	case b >= GB:
		return fmt.Sprintf("%.1f GB", float64(b)/float64(GB))
	case b >= MB:
		return fmt.Sprintf("%.1f MB", float64(b)/float64(MB))
	case b >= KB:
		return fmt.Sprintf("%.1f KB", float64(b)/float64(KB))
	default:
		return fmt.Sprintf("%d B", b)
	}
}
